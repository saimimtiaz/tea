...
function DeleteRequest(id) {
      var conf = prompt("Delivery Feedback between 0 to 5", "");
    debugger;
    conf = parseInt(conf); 
        if (conf != null && conf < 6) {
...
?>
   ...
   <?php if (isset ($_GET["mac"])){ $mac = $_GET["mac"];}else{ $mac = '';} ?>
...
//    $mac = '7C:D1:C3:E1:A9:D9';
    ...
    echo"<span>Rs 0.00 </span>";
        ...
        <div class="col-md-12"> 
            ...
            <center>
                <h6>V 4.0.3(Johar Town)</h6></center>
...
//print_r($items);die;
        ...
        $stock = $products->get_stock($id);
            $out_of_stock = '';
            $msg = '';
            if ($stock == 0){ $out_of_stock = 'blur'; $msg = "<span class='outofstock'>Out Of Stock</span>"; }
            
            
       echo"<div class='item-section' style=''>";
//           if ($stock == 0){ 
...
//echo $products->get_name(2);
 ...
 <span id="mac" style="display:none"><?php echo $mac; ?></span>
  ...
  // overlay section
    ...
    function readRecords() {
        $.get("ajax/readRecords.php?name=<?php if (isset ($_GET["name"])){echo $_GET["name"];} ?>", {},function(data, status) {$(".read").html(data);});}
